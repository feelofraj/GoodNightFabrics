...
$(document).ready(function () {
    $("#popform").submit(function (event) {
        ...
        var $form = $(this),
            ...
            var parameters = { name: $('#name').val(), phone: $('#phone').val(), message: $('#message').val(), mailSubject: "Request from Good Night Fabrics", toMail: "ganesh@citylinkproperties.com" };
        $.ajax({
    ...
    $("#contact").submit(function (event) {
        ...
        var parameters = { name: $('.name').val(), phone: $('.phone').val(), email: $('.email').val(), message: $('.message').val(), mailSubject: "Request from Good Night Fabrics", toMail: "ganesh@citylinkproperties.com" };
        $.ajax({
            url: "https://script.google.com/macros/s/AKfycbxrcGdPjBezMh3x69vM1vx70SoLfs81lHYeG2hla2o-kF854_c/exec",
            method: "POST",
            data: parameters
...
function getMobileOperatingSystem(msgContent) {
    ...
    var msg = "*I am from Chennai*. I am interested in your product. Call me";
    if (msgContent) {
        msg = msgContent;
    }
    var os = "sms:+919500071223?body=" + msg;
    // iOS detection from: http://stackoverflow.com/a/9039885/177710
    if (/iPad|iPhone|iPod/.test(userAgent) && !window.MSStream) {
        os = "sms:+919500071223&body=" + msg;
    }
    location.href = os;
}
function sendMsg() {
